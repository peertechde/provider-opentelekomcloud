...
const (
	errNotSecurityGroup = "managed resource is not a SecurityGroup custom resource"
	errTrackPCUsage     = "cannot track ProviderConfig usage"
	errGetPC            = "cannot get ProviderConfig"
	errGetCPC           = "cannot get ClusterProviderConfig"
	errNewClient        = "cannot create new OTC client"
	errObserve          = "cannot observe SecurityGroup"
	errCreate           = "cannot create SecurityGroup"
	errUpdate           = "cannot update SecurityGroup"
	errDelete           = "cannot delete SecurityGroup"
)
...
func SetupGated(mgr ctrl.Manager, o controller.Options) error {
	...
	}, v1alpha1.SecurityGroupGroupVersionKind)
	return nil
}
...
func Setup(mgr ctrl.Manager, o controller.Options) error {
	...
	if o.MetricOptions != nil && o.MetricOptions.MRStateMetrics != nil {
		...
		if err := mgr.Add(stateMetricsRecorder); err != nil {
			return errors.Wrap(
				err,
				"cannot register MR state metrics recorder for kind v1alpha1.SecurityGroupList",
			)
		}
	}
...
func (c *connector) Connect(
	...
	if !ok {
		return nil, errors.New(errNotSecurityGroup)
	}
...
// external implements managed.ExternalClient for SecurityGroup resources.
...
func (e *external) Observe(
	...
	sg, err := group.Get(e.client, externalName)
	if err != nil {
	...
	cr.Status.AtProvider = v1alpha1.SecurityGroupObservation{
		ID:     sg.ID,
	...
	needsUpdate := e.detectDrift(&cr.Spec.ForProvider, sg)
...
func (e *external) detectDrift(
	spec *v1alpha1.SecurityGroupParameters,
	actual *group.SecurityGroup,
) bool {
...
func (e *external) Create(
	...
	if cr.Spec.ForProvider.Description != nil {
		opts.SecurityGroup.Description = *cr.Spec.ForProvider.Description
	}

	sg, err := group.Create(e.client, opts)
	if err != nil {
		return managed.ExternalCreation{}, errors.Wrap(err, errCreate)
	}

	// Set external name to the Security Group ID
	meta.SetExternalName(cr, sg.ID)
...
func (e *external) Update(
	...
	if cr.Spec.ForProvider.Description != nil {
		opts.SecurityGroup.Description = pointer.Deref(cr.Spec.ForProvider.Description, "")
	}

	_, err := group.Update(e.client, externalName, opts)
	if err != nil {
...
func (e *external) Delete(
	...
	cr, ok := mg.(*v1alpha1.SecurityGroup)
	if !ok {
		return managed.ExternalDelete{}, errors.New(errNotSecurityGroup)
	}
	...
	err := group.Delete(e.client, externalName)
	if err != nil {
		var notFound golangsdk.ErrDefault404
		if errors.As(err, &notFound) {
			return managed.ExternalDelete{}, nil
